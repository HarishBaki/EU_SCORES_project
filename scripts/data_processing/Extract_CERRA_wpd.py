...
if __name__ == "__main__":
    ...
    ds = xr.open_dataset(f'{run_dir}/{region}/{var}{level}.nc',chunks=chunks)
    data = ds.drop('variable')

    rho = 1.225
    wpd = wind_power_density(data['ws'],rho)
    # save to file
    if not os.path.exists(f'{run_dir}/{region}/variablewise_files'):
        os.makedirs(f'{run_dir}/{region}/variablewise_files')
    target_file = f'{run_dir}/{region}/variablewise_files/wpd_{level}.nc'
