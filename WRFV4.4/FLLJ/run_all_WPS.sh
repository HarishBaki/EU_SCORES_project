...
do
    ...
    geogrid_dirs=($(ls -d $root_dir/*_geogrid))
    # loop over the geogrid_dirs
    for geogrid_dir in ${geogrid_dirs[@]}
    do
        ...
        WPS_dir="WPS_"$(basename $geogrid_dir | sed 's/_geogrid//g')
        # create the WPS run directory
        run_dir=$root_dir/$case/$WPS_dir
        mkdir -p $run_dir
        cd $run_dir
            # Call the WPS pipeline here
            if [ $(basename $geogrid_dir | sed 's/_geogrid//g') == 'run_1' ]; then
                grib_prefix='ERA5'
            ...
            elif [ $(basename $geogrid_dir | sed 's/_geogrid//g') == 'run_8' ]; then
                grib_prefix='gfs'
